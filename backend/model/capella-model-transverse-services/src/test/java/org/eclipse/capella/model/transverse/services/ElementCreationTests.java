...
public class ElementCreationTests extends AbstractSemanticTests {
    ...
    public void setFeatureDirectionShouldUpdatePortsConnectedInCascade() {
        Package parent = this.capellaModel.getLogicalArchitecturePerspective().getStructurePackage().getElement();
        PartUsage firstComponent = this.transverseMutationService.createComponent(parent);
        PortUsage firstPort = this.transverseMutationService.createComponentPort(firstComponent, FeatureDirectionKind.OUT);
        PartUsage secondComponent = this.transverseMutationService.createComponent(parent);
        PortUsage secondPort = this.transverseMutationService.createComponentPort(secondComponent, FeatureDirectionKind.IN);
        PartUsage thirdComponent = this.transverseMutationService.createComponent(parent);
        PortUsage thirdPort = this.transverseMutationService.createComponentPort(thirdComponent, FeatureDirectionKind.OUT);
        this.transverseMutationService.createComponentExchange(firstPort, secondPort);
        this.transverseMutationService.createComponentExchange(secondPort, thirdPort);

        this.transverseMutationService.setFeatureDirection(firstPort, FeatureDirectionKind.IN);
    ...
    public void setFeatureDirectionShouldNotUpdatePortsInConnectionCycle() {
        ...
        this.transverseMutationService.createComponentExchange(firstPort, secondPort);
        this.transverseMutationService.createComponentExchange(secondPort, thirdPort);
        this.transverseMutationService.createComponentExchange(thirdPort, firstPort);
