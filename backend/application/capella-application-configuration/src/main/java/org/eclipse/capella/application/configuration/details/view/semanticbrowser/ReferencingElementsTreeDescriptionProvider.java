...
/**
 * Provides the definition of the tree widget for the "Referencing Elements" panel in the Semantic Browser view.
 *
 * @author ntinsalhi
 */
@Service
public class ReferencingElementsTreeDescriptionProvider implements IReferencingElementsTreeDescriptionProvider {

    private static final String WIDGET_ID = "semanticBrowser/incoming";

    private static final String TITLE = "Referencing Elements";
    ...
    private List<?> getIncomingChildren(Object self, EObject root, List<?> ancestors) {
        ...
        if (ancestors.isEmpty()) {
            if (root.equals(self)) {
                result.addAll(this.semanticBrowserService.getReferencingElementsCategories(root));
            }
