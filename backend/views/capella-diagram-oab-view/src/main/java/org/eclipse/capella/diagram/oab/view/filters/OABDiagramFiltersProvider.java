...
/**
 * Filters provider for OAB representation.
 * This class is responsible for providing all diagram filters available on this diagram.
 *
 * @author Jerome Gout
 */
@Service
public class OABDiagramFiltersProvider implements IDiagramFiltersProvider {

    @Override
    public boolean canHandle(IRepresentationDescription representationDescription) {
        return OABViewDiagramDescriptionProvider.DESCRIPTION_NAME.equals(representationDescription.getLabel());
    }

    @Override
    public List<String> getDiagramFilters() {
        return List.of(ShowActivitiesDiagramFilter.ID);
    }
}
