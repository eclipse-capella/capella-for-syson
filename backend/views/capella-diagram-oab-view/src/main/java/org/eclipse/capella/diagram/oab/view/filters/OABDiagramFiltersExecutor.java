...
package org.eclipse.capella.diagram.oab.view.filters;
...
/**
 * Filter executor for all filters of OAB diagrams.
 *
 * @author Jerome Gout
 */
@Service
public class OABDiagramFiltersExecutor implements IDiagramFilterExecutor {
    ...
    public OABDiagramFiltersExecutor(IRepresentationDescriptionLabelSearchService representationDescriptionLabelSearchService, IElementNodeFilteringService elementNodeFilteringService) {
    ...
    public boolean canHandle(IEditingContext editingContext, String representationId, String diagramFilterId) {
        return this.representationDescriptionLabelSearchService.findByRepresentationId(editingContext, representationId)
                .filter(label -> Objects.equals(label, OABViewDiagramDescriptionProvider.DESCRIPTION_NAME))
                .isPresent();
    }

    @Override
    public void execute(IEditingContext editingContext, DiagramContext diagramContext, String diagramFilterId, boolean state) {
        if (Objects.equals(diagramFilterId, ShowActivitiesDiagramFilter.ID)) {
